...
from model.Graphmixer import GraphMixer
from model.modules import MergeLayer
from utils.utils import get_neighbor_sampler, NegativeEdgeSampler, set_random_seed, create_optimizer, convert_to_gpu
from utils.EarlyStopping import EarlyStopping
from utils.DataLoader import get_idx_data_loader, get_link_prediction_data
from utils.load_configs import get_link_prediction_args
from utils.metrics import evaluate_model_link_prediction, get_link_prediction_metrics
...
if __name__ == "__main__":
    with Live() as live:
        ...
        # get dataloader, get a list as an input
        # get the index
        ...
        for run  in range(args.num_runs):
            ...
            for epoch in range(args.num_epochs):
                ...
                #  val_losses, val_metrics for each epoch
            ...
            # load the best model for each run
            ...
            # save metric at the current run
        ...
        # store the average metrics: aucroc and ap
        for metric_name in val_metric_all_runs[0].keys():
            live.log_metric(f'average validate {metric_name}', np.mean([val_metric_single_run[metric_name] for val_metric_single_run in val_metric_all_runs]))

        for metric_name in new_node_val_metric_all_runs[0].keys():
            live.log_metric(f'average new node validate {metric_name}', np.mean([new_node_val_metric_single_run[metric_name] for new_node_val_metric_single_run in new_node_val_metric_all_runs]))
        
        for metric_name in test_metric_all_runs[0].keys():
            live.log_metric(f'average test {metric_name}', np.mean([test_metric_single_run[metric_name] for test_metric_single_run in test_metric_all_runs]))

        for metric_name in new_node_test_metric_all_runs[0].keys():
            live.log_metric(f'average new node test {metric_name}', np.mean([new_node_test_metric_single_run[metric_name] for new_node_test_metric_single_run in new_node_test_metric_all_runs]))
